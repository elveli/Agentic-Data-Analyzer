...
- **Bastion (`aws_instance.bastion`)**: A `t4g.nano` Amazon Linux 2023 instance for reaching the private RDS instance via SSM port-forwarding (no SSH key, no inbound ports — `bastion_sg` has zero ingress rules). The AMI lookup (`data.aws_ami.al2023_arm`) filters for `al2023-ami-2*-arm64` specifically to exclude the `al2023-ami-minimal-*` variant, which doesn't ship the SSM Agent preinstalled — using the minimal AMI by mistake here means the agent never registers with Systems Manager (`aws ssm describe-instance-information` shows nothing). `db_sg`'s ingress allows both `ecs_tasks_sg` and `bastion_sg`. See README "Login to Database Endpoint" for the actual `aws ssm start-session ... AWS-StartPortForwardingSessionToRemoteHost` + `psql` workflow — verified working end-to-end against the live RDS instance.
