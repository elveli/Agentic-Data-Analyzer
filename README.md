...
- **Login to Database Endpoint**: The database is **not** publicly accessible — its security group only allows Postgres traffic from the ECS tasks and a small SSM-managed bastion instance (`aws_instance.bastion` in `terraform/main.tf`), so you can't `psql` into it directly from your laptop. The bastion needs no open inbound ports and no SSH key - access is entirely IAM-gated through AWS Systems Manager:
  1. Get the bastion's instance ID and the real RDS endpoint:
     ```bash
     terraform output bastion_instance_id   # from terraform/, e.g. i-0abc123...
     terraform output vector_db_address     # e.g. agentic-data-analyzer-postgres.xxxxxx.YOUR_AWS_REGION.rds.amazonaws.com
     ```
     Or without Terraform state handy, the AWS CLI equivalents:
     ```bash
     aws ec2 describe-instances --filters "Name=tag:Name,Values=agentic-data-analyzer-bastion" "Name=instance-state-name,Values=running" --region YOUR_AWS_REGION --query 'Reservations[0].Instances[0].InstanceId' --output text
     aws rds describe-db-instances --db-instance-identifier agentic-data-analyzer-postgres --region YOUR_AWS_REGION --query 'DBInstances[0].Endpoint.Address' --output text
     ```
...
- **The next push to `main` will fail, not silently redeploy.** `deploy.yml` builds and pushes the Docker image to ECR *before* running `terraform apply` - but `terraform destroy` also deletes the ECR repository (`force_delete = true`), so the push step fails with `"repository ... does not exist"` and the workflow stops there; `terraform apply` never even runs (verified: it shows up as `skipped` in the run, not executed). The infrastructure stays destroyed, but CI/CD is effectively broken until you manually run `terraform apply` once yourself (from "Cloud Provisioning via Terraform" above) to recreate the ECR repo and everything else - after that, push-to-deploy works normally again. There's no separate on/off switch for the workflow in this repo; this ordering quirk is what actually prevents costs from coming back unintentionally.
