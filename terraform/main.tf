...
resource "aws_ecs_service" "app_service" {
  ...
  # Application Auto Scaling owns desired_count once aws_appautoscaling_target exists below -
  # without this, every `terraform apply` would fight the autoscaler back down to 1.
  lifecycle {
    ignore_changes = [desired_count]
  }
}

# Lets Application Auto Scaling adjust the service's task count instead of it being fixed.
...
# Scales out/in to keep average CPU utilization near 60%, so generating load against the
# app actually does something instead of always hitting the same single Fargate task.
